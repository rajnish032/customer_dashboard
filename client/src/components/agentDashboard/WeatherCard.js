...
const SkeletonLoader = () => (
    <div className=" p-6 pt-4 bg-white rounded-lg shadow-md animate-pulse">
        <div className="h-8 bg-gray-300 rounded w-1/3 mb-4"></div>
        <div className="h-8 bg-gray-300 rounded w-1/2 mb-4"></div>
        <div className="flex space-x-4">
            {Array(7).fill().map((_, index) => (
                <div key={index} className="h-24 w-16 bg-gray-300 rounded"></div>
            ))}
        </div>
    </div>
);
...
const WeatherCard = () => {
    ...
    const fetchWeather = async () => {
        try {
            ...
            const { data } = await axios.get(`https://api.openweathermap.org/data/2.5/forecast?lat=${latitude}&lon=${longitude}&appid=cf259dc5444ca0c3f9ef2676b45aff6d&units=metric`);
            setWeatherData(data);
        } catch (err) {
            setError(err);
        }
    ...
    return (
        <div className=" overflow-auto p-2 min-h-60 h-80 max-h-80  bg-white  transition-all 
    duration-200 ease-in-out cursor-pointer rounded-lg shadow-md border-box">
        <h2 className="text-lg flex items-center gap-2 border-b font-bold px-5 rounded-sm inset-x-0 py-1 text-gray-900 bg-gradient-to-r from-green-500 to-blue-500">
               <TiWeatherPartlySunny /> Weather Card
            </h2>
            ...
            <div className="mt-4 flex xl:gap-11 md:gap-9 gap-7 scrollbar-none  overflow-x-scroll justify-between" style={{ scrollbarWidth: "none" }}>
